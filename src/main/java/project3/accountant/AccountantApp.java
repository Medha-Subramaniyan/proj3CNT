...
public class AccountantApp extends JFrame {
    ...
    private void onConnect() {
        ...
        int ok = JOptionPane.showConfirmDialog(
                this, p, "Accountant Login", JOptionPane.OK_CANCEL_OPTION);
        if (ok != JOptionPane.OK_OPTION) return;

        // Load theaccountant.properties and verify
        Properties acctProps = loadProps("theaccountant.properties");
        String u = acctProps.getProperty("user");
        String pw = acctProps.getProperty("password");
        if (!u.equals(userField.getText()) ||
                !pw.equals(new String(passField.getPassword()))) {
            JOptionPane.showMessageDialog(this,
                    "Invalid credentials", "Login Failed", JOptionPane.ERROR_MESSAGE);
            return;
        }

        // Connect to operationslog
        try {
    ...
    /** Load a .properties file from /props on the classpath */
